...
download_TCGA<-function(cancer_type,dir){
  ...
  setwd(dir)
  cancer_type=str_c('TCGA-',cancer_type)
  query1 <- GDCquery(project = cancer_type,
                     data.category = "Transcriptome Profiling", 
                     data.type = "Gene Expression Quantification", 
                     workflow.type = "STAR - Counts")
  GDCdownload(query1, method = "api", files.per.chunk = 25)
  ...
  GDCdownload(query, method = "api", files.per.chunk = 25)
...
##################################################################################################
manipul_mat<-function(dir,sep,input,row,col){
...
pickcom<-function(dir, op){
  ...
  if (op==T){
    heji=intersect(colnames(lncRNA),colnames(miRNA))#find common samples
    #mRNA=mRNA[,heji]
    miRNA=miRNA[,heji]
    miRNA[is.na(miRNA)]<-0
    lncRNA=lncRNA[,heji]
...
##################################################################################################
geneid_converter<-function(vec,query='en to hugo',ref){
  if (query=="en to hugo"){
    li<-list()
...
##################################################################################################
cal_correlation<-function(interaction_tab,ex1,ex2,filter){
  ...
  if (ncol(interaction_tab)==2){
    ...
    stopCluster(cl)
    res<-cbind(interaction_tab,corr)
    res<-res[abs(res[,3])>filter,]
    return(res)
  }else if (ncol(interaction_tab)==3){#abandoned
    cl = makeCluster(parallel::detectCores() - 1)
    clusterEvalQ(cl,library(ggm))
    clusterEvalQ(cl,library(corpcor))
...
##################################################################################################
cal_delta_pcc<-function(pair_tab,normal_rna,normal_mi,target_rna,target_mi){
  nodes<-unique(c(pair_tab[,1],pair_tab[,2]))
  
  normal_rna<-normal_rna[row.names(normal_rna)%in%nodes,]
  normal_mi<-normal_mi[row.names(normal_mi)%in%nodes,]
  target_rna<-target_rna[row.names(target_rna)%in%nodes,]
  ...
  clusterExport(cl,c("normal_rna","normal_mi","target_rna","target_mi","pair_tab"),envir=environment())
  
  delta_pcc <- parSapply(
    cl,      
    1:nrow(pair_tab),    #whole number of combinations 
    function(i) {
      delta<-numeric(ncol(target_rna))
      
      exp_rna<-c(t(normal_rna[pair_tab[i,2],]))
      exp_mi<-c(t(normal_mi[pair_tab[i,1],]))
      cor=cor(exp_rna, exp_mi, method = "pearson")
      
      for (j in 1:ncol(target_rna)){
        exp_rna<-c(t(normal_rna[pair_tab[i,2],]),target_rna[pair_tab[i,2],j])
        exp_mi<-c(t(normal_mi[pair_tab[i,1],]),target_mi[pair_tab[i,1],j])
        xcor=cor(exp_rna, exp_mi, method = "pearson")
  ...
  stopCluster(cl)
  res<-cbind(pair_tab,t(delta_pcc))
  colnames(res)<-c("mi","rna",colnames(target_rna))
...
##################################################################################################
tnm_separator<-function(dir,clinic_dir,setwd, op){
  setwd(setwd)
  ...
  if (op=="lnm"){#for lymph node metastasis prediction
    #metastasis
    meta=clinic[which(str_sub(clinic$ajcc_pathologic_n,1,2)!='N0'),]
    meta=meta[which(str_sub(meta$ajcc_pathologic_t,1,2)!='T0'),]
    #non-metastasis
  ...
  }else if (op=="m"){#for distant metastasis prediction
    #metastasis
    meta=clinic[which(str_sub(clinic$ajcc_pathologic_m,1,2)=='M1'),]
    meta=meta[which(str_sub(meta$ajcc_pathologic_t,1,2)!='T0'),]
    #non-metastasis
  ...
  dt_positive<-dt[,colnames(dt)%in%(meta$submitter_id)]
  dt_negative<-dt[,colnames(dt)%in%(nonmeta$submitter_id)]
  
  fwrite(dt_positive,str_c('positive_',outputname),sep = ',',row.names = T,quote = F)
  fwrite(dt_negative,str_c('negative_',outputname),sep = ',',row.names = T,quote = F)
}
##################################################################################################
